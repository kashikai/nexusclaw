...
`SUPABASE_SERVICE_ROLE_KEY` is allowed only in local administrative scripts. It
must never use a `NEXT_PUBLIC_*` name, appear in frontend or deployed runtime
code, enter Git, or be printed in logs.
...
3. Apply the migrations only after the preflight is ready.
4. Run the local staging provisioner.
5. Persist the four disposable memberships.
6. Run the rollback-only SQL test matrix.
7. Start the frontend against staging.
8. Run the browser SIWE E2E matrix.
9. Test active-session membership revocation.

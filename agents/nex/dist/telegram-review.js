...
async function generateDraft(techUpdate, postType, language) {
    ...
    for (const model of FREE_MODELS) {
        try {
            ...
            if (!draft)
                throw new Error(`empty content (finish=${data?.choices?.[0]?.finish_reason || 'unknown'})`);
            return draft;
        }
        catch (error) {
...
bot.onText(/\/status/, async (msg) => {
    ...
    await bot.sendMessage(TIAGO_CHAT_ID, `🦞⚡ *NEX STATUS*\n\n` +
...
//# sourceMappingURL=telegram-review.js.map

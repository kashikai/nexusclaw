...
bot.on('callback_query', async (query) => {
    ...
    if (action === 'approve') {
        ...
        await bot.editMessageText(`✅ *APROVADO* — Draft adicionado à fila do Moltbook.\n\n${pending.draft}`, { chat_id: query.message.chat.id, message_id: query.message.message_id, parse_mode: 'Markdown' });
        pendingDrafts.delete(draftId);
    }
    else if (action === 'reject') {
        await bot.editMessageText(`❌ *REJEITADO* — Draft descartado.\n\n${pending.draft}`, { chat_id: query.message.chat.id, message_id: query.message.message_id, parse_mode: 'Markdown' });
        pendingDrafts.delete(draftId);
    }
    else if (action === 'edit') {
        await bot.answerCallbackQuery(query.id, { text: 'Envie o texto editado como resposta.' });
        await bot.sendMessage(query.message.chat.id, `✏️ *EDITAR DRAFT ${draftId}*\n\nEnvie o texto final aprovado:`, { parse_mode: 'Markdown' });
    }
...
bot.on('message', async (msg) => {
    ...
    if (msg.text?.startsWith('/'))
        return; // ignora comandos — já tratados acima
